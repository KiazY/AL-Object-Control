...
async function getReservedObjsSet(objectType: string): Promise<Set<number>> {
    try {
        ...
        const reserved_objects_url = `${(await config_file).sharepoint_baseUrl}/sites/${config_file.sharepoint_siteName}/_api/web/lists/getbytitle('${config_file.sharepoint_listName}')/items?$filter=ObjectType eq '${objectType}'`; // https://m365b784709.sharepoint.com/sites/NW-B2000eBike/_api/web/lists/getbytitle('Object Control')/items?$filter=ObjectType eq '${objectType}'`
